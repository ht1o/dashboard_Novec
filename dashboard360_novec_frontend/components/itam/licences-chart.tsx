...
interface LicPt { month: string; licences_inutilisees: number }

export function LicencesChart({ data }: { data: LicPt[] }) {
  const max = Math.max(...data.map(d => d.licences_inutilisees), 1)
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 10, right: 10, left: -15, bottom: 30 }}>
            ...
            <Tooltip
              ...
              formatter={(v) => [typeof v === "number" ? v : String(v), "Licences inutilisées"]}
            />
            <Bar dataKey="licences_inutilisees" radius={[3, 3, 0, 0]} maxBarSize={36}>
              {data.map((entry, i) => (
                <Cell key={i} fill={entry.licences_inutilisees === max ? "#d13438" : "#e66c37"} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
