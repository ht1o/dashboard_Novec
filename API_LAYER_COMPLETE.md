# Dashboard 360° - Complete FastAPI Layer Implementation

**All code for the API layer with final directory structure and 23 endpoints (Section 5c)**
...
├── auth/
│   ├── __init__.py                       (empty)
│   ├── jwt_handler.py                    (JWT tokens: create, verify, refresh)
│   └── dependencies.py                   (RBAC decorators: get_current_user, check_page_access, check_role, check_role_in)
├── models/
│   ├── __init__.py                       (empty)
│   └── schemas.py                        (Pydantic models for all 8 domains + responses)
├── db/
│   ├── __init__.py                       (empty)
│   └── connection.py                     (SQL Server engine + connection pooling)
└── routers/
    ├── __init__.py                       (empty)
    ├── auth.py                           (POST /auth/token, POST /auth/refresh)
    ├── health.py                         (GET /api/health)
    ├── pipeline_management.py            (POST /api/pipeline/trigger)
    ├── risk_score.py                     (GET /api/risk-score/latest, /history)
    ├── anomalies_alerts.py               (GET /api/anomalies, /alerts/active, /recommendations + PATCH)
    ├── forecasts.py                      (GET /api/forecast/{domain})
    ├── dashboards.py                     (GET /api/dashboard/* for 9 pages)
    └── audit.py                          (GET /api/audit)
```

---

# CODE FILES - COPY IN THIS ORDER
...
## 12. api/routers/pipeline_management.py
...
## 14. api/routers/anomalies_alerts.py
...
## DEPENDENCIES: requirements.txt (ADD THESE LINES)
...
## COMPLETE ENDPOINT REFERENCE (23 TOTAL)

### Auth (2)
```
POST   /auth/token                 → TokenResponse (access + refresh token)
POST   /auth/refresh               → TokenResponse (new access token)
```

### Health & Pipeline (2)
```
GET    /api/health                 → {status, service, timestamp, last_pipeline_run}
POST   /api/pipeline/trigger       → {status, message, triggered_by, timestamp} (dsi only)
```

### Risk Score (2)
```
GET    /api/risk-score/latest      → ITRiskSummary {global_risk_score, by_domain{}}
GET    /api/risk-score/history     → {domain, period_days, data_points, history[]}
```

### Anomalies & Alerts (4)
```
GET    /api/anomalies              → List[AnomalyAlert] (?domain, ?date, ?rag_status)
GET    /api/alerts/active          → List[AnomalyAlert] (Prophet + ZScore combined)
GET    /api/recommendations        → List[Recommendation] (?role, ?status)
PATCH  /api/recommendations/{id}   → {status, recommendation_id, new_status, updated_by}
```

### Forecasts (1)
```
GET    /api/forecast/{domain}      → Forecast {entity, kpi, domain, forecast_points[], confidence_level}
```

### Dashboards (9)
```
GET    /api/dashboard/executive    → {score, alerts, recommendations} (Gold only)
GET    /api/dashboard/infrastructure → {metrics, anomalies, forecast} (Silver + Gold)
GET    /api/dashboard/itsm         → {metrics, anomalies, forecast} (Silver + Gold)
GET    /api/dashboard/cybersec     → {metrics, anomalies, forecast} (Silver + Gold)
GET    /api/dashboard/applications → {metrics, anomalies, forecast} (Silver + Gold)
GET    /api/dashboard/itam         → {metrics, anomalies, forecast} (Silver + Gold)
GET    /api/dashboard/parc_auto    → {metrics, anomalies, forecast} (Silver + Gold)
GET    /api/dashboard/maintenance  → {metrics, anomalies, forecast} (Silver + Gold)
GET    /api/dashboard/finance      → {metrics, anomalies, forecast} (Silver + Gold)
```

### Audit (1)
```
GET    /api/audit                  → {page, page_size, total_records, data[]} (dsi only)
```
...
2. **Create all 18 files** in the order listed above
...
4. **Configure .env** with your SQL Server credentials
...
✅ **10 roles** with 10 page-level access patterns  
✅ **JWT tokens** - 15 min access + 7 day refresh  
✅ **Role-based RBAC** - Page and role-level checks  
✅ **23 endpoints** - All section 5c requirements  
✅ **Type-safe** - Pydantic validation on all requests/responses  
✅ **SQL Server** - Connection pooling, parameterized queries  
...
- [ ] Implement Gold.users auth (replace demo credentials)
